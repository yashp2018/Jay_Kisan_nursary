...
export default function BookingManagementPage() {
  ...
  const [includeAdditionalCrops, setIncludeAdditionalCrops] = useState(false);
  const [additionalCropQty, setAdditionalCropQty] = useState(1);
  const [additionalCropRate, setAdditionalCropRate] = useState(0);
  const additionalCropAmount = additionalCropQty * additionalCropRate;
...
const generateSowingSchedules = async () => {
  ...
  try {
    ...
    console.error("Failed to generate sowing schedules:", error);
    const errorMessage = error.response?.data?.message || error.message || "Failed to generate sowing schedules";
  ...
  const openPayModal = (booking) => {
    ...
    setIncludeAdditionalCrops(false);
    setAdditionalCropQty(1);
    setAdditionalCropRate(0);
  ...
  const submitPayment = async () => {
    try {
      ...
      let paymentAmount = Number(payAmount);
      let notes = payNotes;
      
      // Add additional crop amount if included
      if (includeAdditionalCrops && additionalCropAmount > 0) {
        paymentAmount += Number(additionalCropAmount);
        notes = `${payNotes ? payNotes + '\n' : ''}Additional Crops: ${additionalCropQty} x ₹${additionalCropRate} = ₹${additionalCropAmount}`;
      }
      
      if (isNaN(paymentAmount) || paymentAmount <= 0) return alert("Enter a valid amount");

      // Prevent paying more than remaining (only check if not including additional crops)
      if (!includeAdditionalCrops) {
      ...
      await axios.post(`/bookings/${payBookingId}/pay`, {
        ...
        method: payMethod,
        notes: notes,
        includeAdditionalCrops,
        additionalCropAmount: includeAdditionalCrops ? additionalCropAmount : 0,
        additionalCropQty: includeAdditionalCrops ? additionalCropQty : 0,
        additionalCropRate: includeAdditionalCrops ? additionalCropRate : 0
      });
  ...
  const fetchBookingsForTab = async () => {
    ...
    try {
      ...
      } else if (Array.isArray(resp.data?.bookings)) {
        bookingsData = resp.data.bookings;
      } else if (Array.isArray(resp.data?.data)) {
        bookingsData = resp.data.data;
      } else {
  ...
  const filteredBookings = bookings.filter((b) => {
    ...
    const registrationNo = (b.farmer?.registrationNo || "").toLowerCase();
    return farmerName.includes(q) || crop.includes(q) || bookingId.includes(q) || registrationNo.includes(q);
  });
        ...
        <div className="bg-green-700 text-white p-5 text-center relative overflow-hidden">
          ...
          <div className="flex justify-between items-center mb-4 relative">
            ...
            <div className="flex gap-2">
              {/* Generate Sowing Schedules Button */}
              <button
                onClick={generateSowingSchedules}
              ...
              >
                <i className={`fas ${generatingSchedules ? "fa-spinner fa-spin" : "fa-calendar-plus"} mr-2`}></i>
                {generatingSchedules ? "Generating..." : "Generate Schedules"}
              </button>
              
              {/* New Booking Button - Only for staff */}
              {user?.role === 'staff' && (
                <button
                  onClick={() => {
                    setShowModal(true);
        ...
        <div className="p-4 md:p-6">
          ...
          <div className="mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div className="relative w-full md:w-auto">
              ...
              <input
                type="text"
                placeholder="Search by farmer name, crop, or registration no..."
                value={search}
                onChange={(e) => {
                    ...
                    return (
                      <tr key={(booking._id || booking.bookingId).toString()} className="border-b hover:bg-green-50">
                        ...
                        <td className="p-3 font-mono text-sm">
                          {booking.farmer?.registrationNo || "N/A"}
                        </td>
                        <td className="p-3">
                          {booking.varieties?.map(v => String(v?.name || v || '')).join(", ") || "—"}
                        </td>
                        <td className="p-3">{booking.varieties?.map(v => String(v?.quantity || v || '')).join(", ") || "—"}</td>
                        ...
                        <td className="p-3 flex gap-2 justify-center flex-wrap">
                          {booking._id ? (
                            <button
                              ...
                              className="bg-red-500 text-white px-3 py-1 rounded-lg flex items-center text-sm"
                            >
                              <i className="fas fa-trash mr-1"></i> Delete
                            </button>
                          ) : (
                            <button
                              className="bg-gray-300 text-white px-3 py-1 rounded-lg cursor-not-allowed text-sm"
                              disabled
                            >
                              Delete
                            </button>
                                ...
                                <button
                                  ...
                                  className={`${
                                    nextStatus === "sowing"
                                      ? "bg-blue-500"
                                      : "bg-green-500"
                                  } text-white px-3 py-1 rounded-lg flex items-center text-sm`}
                                >
                          ...
                          {(() => {
                            ...
                            <button
                              ...
                              className="bg-amber-500 text-white px-3 py-1 rounded-lg flex items-center text-sm"
                            >
                              <i className="fas fa-indian-rupee-sign mr-1"></i> Pay
                            </button>
                          )}
                        </td>
                      </tr>
                    ...
                    <div>
                      <label className="flex items-center space-x-2 cursor-pointer">
                        <input
                          ...
                          checked={includeAdditionalCrops}
                          onChange={(e) => setIncludeAdditionalCrops(e.target.checked)}
                        />
                        <span className="text-sm font-medium">Include additional crops</span>
                      </label>
                    </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            ...
                            <input
                              type="number"
                              className="w-full border rounded-lg p-2"
                              value={additionalCropQty}
                              min={1}
                              onChange={(e) => setAdditionalCropQty(Number(e.target.value) || 1)}
                            />
                          </div>
                          <div>
                            ...
                            <input
                              type="number"
                              className="w-full border rounded-lg p-2"
                              value={additionalCropRate}
                              min={0}
                              step="0.01"
                              onChange={(e) => setAdditionalCropRate(Number(e.target.value) || 0)}
                            />
                          </div>
                        </div>
                        <div className="bg-blue-50 p-3 rounded-lg">
                          <div className="grid grid-cols-2 gap-2 text-sm text-gray-700">
                            <div>Additional Crops:</div>
                            <div className="text-right">{additionalCropQty} x ₹{additionalCropRate.toFixed(2)}</div>
                            <div className="font-medium">Subtotal:</div>
                            <div className="font-medium text-right">₹{additionalCropAmount.toFixed(2)}</div>
                            <div className="col-span-2 border-t border-gray-300 my-1"></div>
                            <div className="font-semibold">Total Payable:</div>
                            <div className="font-semibold text-right">
                              ₹{(Number(payAmount) + Number(additionalCropAmount || 0)).toFixed(2)}
                            </div>
                          </div>
                        </div>
                      </div>
